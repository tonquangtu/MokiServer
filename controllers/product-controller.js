...
exports.postCommentProduct = (req, res) => {
  const data = req.body;
  const validateResult = validateComment(data);
  if (!validateResult.valid) {
    helpers.sendResponse(res, validateResult.statusCode, validateResult.responseData);
  } else {
    const { productId, comment, index } = validateResult.responseData;
...
function validateComment(commentParams) {
  if (!commentParams.productId || !commentParams.comment || !commentParams.index) {
    return {
      valid: false,
      statusCode: constants.statusCode.notFound,
      responseData: constants.response.paramNotEnough,
